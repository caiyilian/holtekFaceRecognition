...
0-小程序订阅（本地服务器或esp32的）消息，可能的消息内容如下
    ...
    消息内容为<{"mode":"update"}>表示本地服务器提醒小程序当前核酸检测记录已经更新（小程序可以选择更新一个当前的核酸检测记录）
    
1-本地服务器订阅（小程序或esp32的）消息，可能的消息内容如下
    消息内容为<get_record:页码> 表示小程序想获取截至目前的所有核酸检测记录中某一页，
    消息内容为<face_recognition:ip地址:设备名> 表示esp32请求本地服务器对它当前的图片进行人脸识别
...
2-esp32订阅消息，根据消息内容来判断是要测试在线情况、人脸识别等等情况
    ...
    消息内容为<face_recognition>表示合泰想对当前这个esp32捕获的图片进行人脸识别
    消息内容为<get_temperature:温度> 表示合泰把温度发给esp32想让它把温度转发给本地服务器
